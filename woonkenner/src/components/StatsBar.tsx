...
import { Home, Calculator, Euro, TrendingUp, Leaf, type LucideIcon } from "lucide-react";

const iconMap: Record<string, LucideIcon> = { Home, Calculator, Euro, TrendingUp, Leaf };
          ...
          {stats.map((stat, i) => {
            ...
            return (
              <div key={i} className={`flex items-center gap-4 transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ transitionDelay: `${i * 100}ms` }}>
                <div className="bg-blue-50 p-3 rounded-xl shrink-0">{Icon && <Icon size={24} className="text-blue-600" />}</div>
